...
case $install in
    ...
    F=`sudo find | egrep nvidia-450_cuda-11.0_cudnn-8.0.2_setup.sh`
    F=(${F// .// })
    ...
    ;;
    3)
    echo -e "\n${WHITE_TXT}cudnn 8.0.2 installation${NO_COLOR}"
    Fcudnn=`sudo find  | egrep cudnn-11.0-linux-ppc64le-v8.0.2.39.tgz`
    if [ $? -eq 0 ]; then
        Fcudnn=(${Fcudnn// .// })
        ...
        echo -e "${WHITE_TXT}Please make sure you have the correct file (cudnn-11.0-linux-ppc64le-v8.0.2.39.tgz) then try to install cudnn once more.${NO_COLOR}"
        exit 0
    fi
    ...
    if [ $? -eq 0 ]; then
        echo -e "\n${GREEN_TXT}cudnn 8.0.2 installation is done${NO_COLOR}"
    else
        echo -e "\n${RED_TXT}An error occurred, couldn't install cudnn 8.0.2. You could try to install cudnn manually::${NO_COLOR}"
