...
CUDA_VERSOIN=10.1
CUDNN_VERSOIN=7.6.5
CUDA_FILE=cuda_10.1.243_418.87.00_linux.run
CUDNN_FILE=cudnn-10.1-linux-x64-v7.6.5.32.tgz
CUDA_URL=http://developer.download.nvidia.com/compute/cuda/10.1/Prod/local_installers/cuda_10.1.243_418.87.00_linux.run
CUDNN_URL=https://developer.nvidia.com/compute/machine-learning/cudnn/secure/7.6.5.32/Production/10.1_20191031/cudnn-10.1-linux-x64-v7.6.5.32.tgz
FILE_NAME=${0##*/}
...
echo -e "\n${WHITE_TXT}Hi, what would you like to install?\n  press (1) for Nvidia driver 418,\n  press (2) for cuda 10.1,\n  press (3) for cudnn 7.6.5,\n  press (4) for all of them at once,\nwhat is your choice? <1/2/3/4> ${NO_COLOR}\c"
...
case $install in
    ...
    if [ $? -eq 0 ]; then
        ...
        echo -e "\n${WHITE_TXT}You could try to find the right path to cuda_10.1.243_418.87.00_linux.run, then go to the file location and run it with:${NO_COLOR}"
